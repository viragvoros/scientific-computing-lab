function p = adams_moulton_l2(ODE, p0, delta_t, t_end)
...
for i = 1:N - 1
    % Definition: y(n+1) = y(n) + (y'(n) + 7 * (1 - y(n) / 10) * y(n+1)) * Δt / 2
    % We change it to 0 = y(n) + (y'(n) + 7 * (1 - y(n) / 10) * y(n+1)) * Δt / 2 - y(n+1)
    ...
    func = @(y) p(i) + (ODE(p(i)) + 7 * (1 - p(i) / 10) * y) * delta_t / 2 - y;
    dfunc = @(y) 7 * delta_t / 2 * (1 - p(i) / 10) - 1;
