...
[t,p]=odeRUNGE(@dpdt,t_init,t_last,delta_t(idelta_t),p_init); % t,p output variables  
...
% Displaying error for Huen method
errors_RUNGE_KUTTA=[delta_t; E; factor]
%--------------------------------------------------------------------------
...
function [t,p] = odeRUNGE(ODE,t_init,t_last,delta_t,p_init)
...
for i=1:N-1
    t(i+1)=t(i) + delta_t;
    Y1 = ODE(t(i), p(i));
    Y2 = ODE(t(i)+0.5*delta_t,p(i)+0.5*Y1*delta_t);
    Y3 = ODE(t(i)+0.5*delta_t,p(i)+0.5*Y2*delta_t);
    Y4 = ODE(t(i)+delta_t,p(i)+Y3*delta_t);
