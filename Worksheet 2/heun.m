...
[t,p]=odeHEUN(@dpdt,t_init,t_last,delta_t(idelta_t),p_init); % t,p output variables  
...
% Plotting Huen method with different values for step size
...
function [t,p] = odeHEUN(ODE,t_init,t_last,delta_t,p_init)
...
for i=1:N-1
    t(i+1)=t(i) + delta_t;
    Y1 = ODE(t(i), p(i));                   % y'(n)
    Y2 = ODE(t(i)+delta_t, p(i)+Y1*delta_t);% y'(n+1)
